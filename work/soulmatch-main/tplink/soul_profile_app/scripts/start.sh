...
release_port() {
  local i=0
  while [ "$i" -lt 15 ]; do
    if [ "$OS_NAME" != "Darwin" ] && command -v fuser >/dev/null 2>&1; then
...
if [ ! -x "$NEXT_BIN" ]; then
  echo "[start.sh] 未找到本地 Next 可执行文件: $NEXT_BIN" >&2
  echo "[start.sh] 请先在项目目录执行 npm install" >&2
  exit 1
fi

exec "$NEXT_BIN" start -H 0.0.0.0 -p "$PORT" "$@"
