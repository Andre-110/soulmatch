...
export function stubArticleFromLegacy(params: {
  ...
  return {
    ...
    section4: {
      ...
      finalCard: {
        intro: '当前为兜底版总结卡，优先保留可回查内容，不补无依据设定。',
        summaryLines: [
          mbti ? `你的人格：${mbti}` : '你的人格：待模型补全',
          title ? `你的设定：${title}` : '你的设定：待模型补全',
          overall ? `本轮重点：${pickSnippet(overall, 30)}` : '本轮重点：公开资料与问卷已纳入',
        ],
        closingLine: '当前已完成本轮材料整理；如配置大模型，可生成更完整的终版收束。',
      },
      dayParts: [
        {
          order: 1,
